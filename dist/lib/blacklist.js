...
export async function isTokenRevoked(userId, issuedAt) {
    // Compare entirely in SQL to avoid timezone mismatch between MySQL NOW()
    // (server timezone, e.g. IST) and JavaScript Date.now() (UTC).
    const row = await db.prepare("SELECT UNIX_TIMESTAMP(revoked_at) > ? AS revoked FROM token_blacklist WHERE user_id = ? AND expires_at > NOW() LIMIT 1").get(issuedAt ?? 0, userId);
    if (!row)
        return false;
