...
async function logEmail(recipientId, recipientEmail, subject, status, errorMessage) {
    try {
...
export async function sendPasswordResetEmailToUser(user, resetLink) {
    if (!isEmailConfigured())
        return;
    const subject = 'Password Reset Request';
    const html = `<h2>Password Reset</h2><p>You requested a password reset. Click the link below to set a new password:</p><p><a href="${resetLink}">${resetLink}</a></p><p>If you did not request this, you can safely ignore this email.</p>`;
...
export async function sendLeaveNotification(leave, action, recipient) {
    if (!isEmailConfigured())
        return;
...
export async function sendTaskAssignment(task, assignee) {
    if (!isEmailConfigured())
        return;
    const base = process.env.APP_URL ?? 'http://localhost:3000';
...
export async function sendDocumentReady(document, requester) {
    if (!isEmailConfigured())
        return;
    const base = process.env.APP_URL ?? 'http://localhost:3000';
...
export async function sendAuditAlert(alert) {
    if (!isEmailConfigured())
        return;
...
export async function sendSecurityAlertEmail(email, ip) {
    if (!isEmailConfigured())
        return;
