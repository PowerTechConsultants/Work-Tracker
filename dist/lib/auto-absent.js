...
async function acquireLock() {
    ...
    const result = await db.prepare("UPDATE app_settings SET value = ? WHERE `key` = 'auto_absent_lock' AND (value = '' OR CAST(value AS UNSIGNED) < ?)").run(String(now), cutoff);
    if (result.changes > 0)
        return true;
...
async function skipUsersForHoliday(date) {
    const holidays = await db.prepare('SELECT id FROM holidays WHERE date = ?').all(date);
    if (holidays.length === 0)
        return [];
    const holidayIds = holidays.map((h) => h.id);
    const placeholders = holidayIds.map(() => '?').join(',');
    const assignees = await db.prepare(`SELECT user_id FROM holiday_assignees WHERE holiday_id IN (${placeholders})`).all(...holidayIds);
    let hasCompanyWide = false;
    for (const h of holidays) {
        const cnt = (await db.prepare('SELECT COUNT(*) as c FROM holiday_assignees WHERE holiday_id = ?').get(h.id)).c;
        if (cnt === 0) {
            hasCompanyWide = true;
            break;
        }
    ...
    return assignees.map((r) => r.user_id);
}
async function isCompanyWideHoliday(date) {
    const holidays = await db.prepare('SELECT id FROM holidays WHERE date = ?').all(date);
    if (holidays.length === 0)
        return false;
    for (const h of holidays) {
        const assigneeCount = (await db.prepare('SELECT COUNT(*) as c FROM holiday_assignees WHERE holiday_id = ?').get(h.id)).c;
        if (assigneeCount > 0)
            return false;
    }
    return true;
}
async function markAbsentForDay(date, skipUserIds) {
    let params = [];
    let skipClause = '';
    ...
    const existingIds = new Set((await db.prepare(`SELECT user_id FROM attendance WHERE date = ?`).all(date)).map((r) => r.user_id));
    const activeUsers = await db.prepare(`SELECT id, joining_date FROM users WHERE status = 'active'${skipClause}`).all(...params);
    const toInsert = activeUsers.filter((u) => {
        if (existingIds.has(u.id))
            return false;
        if (u.joining_date && u.joining_date > date)
            return false;
        return true;
    });
...
async function processDay(date) {
    if (isSundayIST(date))
        return;
    if (await isCompanyWideHoliday(date))
        return;
...
async function backfillAbsentDates() {
    ...
    try {
        ...
        if (cursor > cutoff)
            return;
        const runDay = db.transaction(async (date) => {
