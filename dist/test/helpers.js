...
export async function createUser(opts = {}) {
    ...
    await db.prepare('INSERT INTO users (id, employee_id, first_name, last_name, email, password_hash, role, status, gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(user.id, user.employeeId, user.firstName, user.lastName, user.email, hash, user.role, 'active', opts.gender ?? null);
    return user;
}
