...
describe('database migrations', () => {
    ...
    it('creates all core tables', async () => {
        const rows = (await db.prepare('SELECT TABLE_NAME as name FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = DATABASE()').all());
    ...
    it('admin bootstrap is idempotent', async () => {
        const before = (await db.prepare("SELECT COUNT(*) as c FROM users WHERE role IN ('director','hr')").get());
        await ensureAdminBootstrap();
        const after = (await db.prepare("SELECT COUNT(*) as c FROM users WHERE role IN ('director','hr')").get());
