...
function mysqlToSqlite(sql) {
    ...
    s = s.replace(/INSERT\s+INTO\s+(`?)(\w+)\1[\s\S]*?ON DUPLICATE KEY UPDATE([\s\S]+?)(;|$)/gi, (_m, _q, table, setClause, end) => {
        const target = CONFLICT_TARGETS[table];
        if (!target)
            throw new Error(`[DB] Unsupported ON DUPLICATE KEY UPDATE for table ${table}`);
    ...
    s = s.replace(/CONVERT_TZ\(\s*([^,]+?)\s*,\s*'[^']*'\s*,\s*'([+-])(\d{2}):(\d{2})'\s*\)/gi, (_m, col, sign, hh, mm) => {
...
const db = {
    ...
    exec: async (sql) => {
        ...
        for (const stmt of stmts) {
            ...
            try {
                ...
                const msg = e.message ?? '';
                if (/already exists|duplicate column|duplicate/i.test(msg))
                    continue;
    ...
    transaction: (fn) => {
        return async (...args) => {
            ...
            const prev = txnLock;
            let release;
            txnLock = new Promise((r) => (release = r));
            await prev;
            txnDepth++;
            try {
...
async function hasColumn(table, column) {
    ...
    const rows = sqlite.prepare(`SELECT name FROM pragma_table_info('${table}')`).all();
...
const migrations = [
    ...
    },
    {
        ...
        up: async () => {
            ...
            try {
                await db.exec(`UPDATE attendance SET notes = REPLACE(notes, 'paid leave', 'proposal leave') WHERE notes LIKE '%paid leave%'`);
            }
            catch (e) {
    ...
    },
    {
      ...
      `);
            await db.prepare(`REPLACE INTO file_retention_policies (id, mime_pattern, retention_days) VALUES (?, ?, ?), (?, ?, ?), (?, ?, ?), (?, ?, ?)`).run('1', 'image/*', 365, '2', 'application/pdf', 730, '3', 'text/*', 365, '4', '*', 180);
        },
    },
    {
        version: 13,
      ...
      `);
            ...
            if (!existing) {
                ...
                await db.prepare("INSERT INTO password_policies (id, min_length, require_uppercase, require_lowercase, require_number, require_special, max_age_days, history_count) VALUES (?, 12, 1, 1, 1, 1, 90, 5)").run(randomUUID());
            }
        },
    },
    {
