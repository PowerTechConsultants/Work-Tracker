...
export async function nextEmployeeId() {
    return await db.transaction(async () => {
        const row = (await db.prepare("SELECT MAX(CAST(SUBSTR(employee_id, 5) AS UNSIGNED)) as max_seq FROM users").get());
...
export class UsersService {
    static async list(input) {
        ...
        if (search) {
            ...
            conditions.push("(first_name LIKE ? ESCAPE '\\\\' OR last_name LIKE ? ESCAPE '\\\\' OR email LIKE ? ESCAPE '\\\\' OR employee_id LIKE ? ESCAPE '\\\\')");
            params.push(`%${escaped}%`, `%${escaped}%`, `%${escaped}%`, `%${escaped}%`);
        }
        const where = conditions.length > 0 ? `WHERE ${conditions.join(' AND ')}` : '';
        const count = (await db.prepare(`SELECT count(*) as c FROM users ${where}`).get(...params)).c;
    ...
    static async getById(id) {
        const u = await db.prepare('SELECT id, employee_id, first_name, last_name, email, role, status, department_id, designation, phone_number, joining_date, dob, gender, father_name, nationality, qualification, address_street, address_city, address_state, address_pincode, profile_picture_url, two_factor_enabled, last_login_at, created_at, updated_at FROM users WHERE id = ?').get(id);
        if (!u)
    ...
    static async update(id, input, requesterRole) {
        if (!await db.prepare('SELECT id FROM users WHERE id = ?').get(id))
            throw new AppError(404, 'User not found');
    ...
    static async delete(id, input) {
        if (input.confirm !== 'DELETE')
            throw new AppError(400, 'Deleting a user requires confirm=DELETE in the request body');
        if (!await db.prepare('SELECT id FROM users WHERE id = ?').get(id))
            throw new AppError(404, 'User not found');
        const tasks = (await db.prepare('SELECT COUNT(*) as c FROM tasks WHERE created_by_id = ?').get(id)).c;
        if (tasks > 0)
            throw new AppError(409, `Cannot delete user: they created ${tasks} task(s). Delete or reassign those tasks first.`);
        const holidays = (await db.prepare('SELECT COUNT(*) as c FROM holidays WHERE created_by = ?').get(id)).c;
        if (holidays > 0)
            throw new AppError(409, `Cannot delete user: they created ${holidays} holiday(s). Delete those holidays first.`);
        await db.prepare('DELETE FROM users WHERE id = ?').run(id);
        return { message: 'User deleted' };
    }
    static async getStats() {
        const stats = await db.prepare('SELECT role, COUNT(*) as count, SUM(CASE WHEN status = \'active\' THEN 1 ELSE 0 END) as active FROM users GROUP BY role').all();
        const total = stats.reduce((sum, r) => sum + r.count, 0);
        const active = stats.reduce((sum, r) => sum + (r.active || 0), 0);
        const roleMap = Object.fromEntries(stats.map((r) => [r.role, r.count]));
