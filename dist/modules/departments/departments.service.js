...
export class DepartmentsService {
    ...
    static async getById(id) {
        const d = await db.prepare('SELECT * FROM departments WHERE id = ?').get(id);
        if (!d)
    ...
    static async create(input) {
        ...
        await db.transaction(async () => {
            if (await db.prepare('SELECT id FROM departments WHERE name = ?').get(input.name))
    ...
    static async delete(id) {
        await this.getById(id);
        const count = (await db.prepare('SELECT count(*) as c FROM users WHERE department_id = ?').get(id)).c;
        if (count > 0)
    ...
    static async getStats() { return { total: (await db.prepare('SELECT count(*) as c FROM departments').get()).c }; }
}
