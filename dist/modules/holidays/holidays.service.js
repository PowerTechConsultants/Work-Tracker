...
async function notifyUsers(userIds, senderId, title, message, link) {
    if (!userIds || userIds.length === 0)
        return;
    const insert = db.prepare('INSERT INTO notifications (id, recipient_id, sender_id, title, message, type, link) VALUES (?, ?, ?, ?, ?, ?, ?)');
    const insertMany = await db.transaction(async (chunk) => {
        for (const uid of chunk)
            await insert.run(uuid(), uid, senderId, title, message, 'holiday', link ?? '/holidays');
    });
    const CHUNK = 500;
    ...
    try {
        for (const uid of userIds)
            getIO().to(`user:${uid}`).emit('notification:new', { title, message, type: 'holiday', link: link ?? '/holidays' });
    }
    catch (e) {
        console.error('[Holidays] Notification socket emit failed:', e);
    }
}
...
export class HolidaysService {
    static async create(userId, input) {
        ...
        const affectedUsers = hasSpecificUsers
            ? input.userIds
            : (await db.prepare("SELECT id FROM users WHERE status = 'active'").all()).map((u) => u.id);
        const result = await (await db.transaction(async () => {
            // Check for existing holiday inside transaction to prevent race condition
            const existing = await db.prepare('SELECT id, name FROM holidays WHERE date = ?').get(input.date);
            if (existing)
                throw new AppError(409, `A holiday already exists for ${input.date} (${existing.name})`);
            const id = uuid();
            ...
            for (let i = 0; i < affectedUsers.length; i += CHUNK) {
                ...
                const params = [];
                for (const uid of chunk) {
                    params.push(uuid(), uid, input.date, 'holiday', input.name);
                }
        ...
        try {
            ...
            for (const a of admins)
                getIO().to(`user:${a.id}`).emit('holiday:created', result);
            for (const uid of affectedUsers)
                getIO().to(`user:${uid}`).emit('holiday:created', result);
        }
        catch (e) {
            console.error('[Holidays] Socket emit failed:', e);
        }
        return result;
    }
    static async list(input) {
        ...
        const count = (await db.prepare(`SELECT count(*) as c FROM holidays h${whereClause}`).get(...params)).c;
        const holidays = await db.prepare(`SELECT h.*, u.first_name, u.last_name FROM holidays h JOIN users u ON h.created_by = u.id${whereClause} ORDER BY h.date DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
        const result = [];
        if (holidays.length > 0) {
            const holidayIds = holidays.map((h) => h.id);
            const placeholders = holidayIds.map(() => '?').join(',');
            const allAssignees = await db.prepare(`SELECT ha.holiday_id, u.id, u.first_name, u.last_name FROM holiday_assignees ha JOIN users u ON ha.user_id = u.id WHERE ha.holiday_id IN (${placeholders})`).all(...holidayIds);
            for (const h of holidays) {
                const assignees = allAssignees.filter((a) => a.holiday_id === h.id);
                result.push({
                    id: h.id, date: h.date, name: h.name, type: h.type,
                    createdBy: { id: h.created_by, firstName: h.first_name, lastName: h.last_name },
                    assignees: assignees.length > 0 ? assignees.map((a) => ({ id: a.id, firstName: a.first_name, lastName: a.last_name })) : null,
    ...
    static async getById(id) {
        const h = await db.prepare('SELECT id, name, date, type, created_by, created_at, updated_at FROM holidays WHERE id = ?').get(id);
        if (!h)
    ...
    static async delete(id, input, userId) {
        if (input.confirm !== 'DELETE')
            throw new AppError(400, 'Deleting a holiday requires confirm=DELETE in the request body');
        const h = await db.prepare('SELECT id, name, date, type, created_by, created_at, updated_at FROM holidays WHERE id = ?').get(id);
        if (!h)
            throw new AppError(404, 'Holiday not found');
        const affectedUsers = await db.prepare('SELECT user_id FROM holiday_assignees WHERE holiday_id = ?').all(id);
        const userIds = affectedUsers.length > 0
            ? affectedUsers.map((a) => a.user_id)
            : (await db.prepare("SELECT id FROM users WHERE status = 'active'").all()).map((u) => u.id);
        await db.transaction(async () => {
            ...
            if (targetUserIds.length > 0) {
                ...
                const leavesForDate = await db.prepare(`SELECT user_id, id as leave_id FROM leaves WHERE status = 'approved' AND start_date <= ? AND end_date >= ? AND user_id IN (${ph})`).all(h.date, h.date, ...targetUserIds);
                const leaveMap = new Map();
                for (const l of leavesForDate)
                    leaveMap.set(l.user_id, l.leave_id);
                ...
                const allActive = await db.prepare("SELECT id FROM users WHERE status = 'active'").all();
        ...
        try {
            const admins = await db.prepare("SELECT id FROM users WHERE role IN ('director', 'hr')").all();
            for (const a of admins)
                getIO().to(`user:${a.id}`).emit('holiday:deleted', { id, date: h.date });
            for (const uid of userIds)
                getIO().to(`user:${uid}`).emit('holiday:deleted', { id, date: h.date });
        }
        catch (e) {
            console.error('[Holidays] Socket emit failed:', e);
        }
