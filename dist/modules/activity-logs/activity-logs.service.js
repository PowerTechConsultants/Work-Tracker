...
export class ActivityLogsService {
    ...
    static async list(input) {
        ...
        const params = [];
        if (actorId) {
            conds.push('al.actor_id = ?');
            params.push(actorId);
        }
        if (entityType) {
            conds.push('al.entity_type = ?');
            params.push(entityType);
        }
        if (entityId) {
            conds.push('al.entity_id = ?');
            params.push(entityId);
        }
        const where = conds.length > 0 ? `WHERE ${conds.join(' AND ')}` : '';
        const count = (await db.prepare(`SELECT count(*) as c FROM activity_logs al ${where}`).get(...params)).c;
    ...
    static async getById(id) {
        const log = await db.prepare('SELECT * FROM activity_logs WHERE id = ?').get(id);
        if (!log)
