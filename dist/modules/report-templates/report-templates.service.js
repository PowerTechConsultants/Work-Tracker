...
export class ReportTemplatesService {
    static async create(userId, input) {
        const id = uuid();
        await db.prepare(`INSERT INTO report_templates (id, user_id, name, description, type, fields, is_default)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, userId, input.name, input.description ?? null, input.type, JSON.stringify(input.fields ?? []), input.isDefault ? 1 : 0);
        return mapTemplate(await db.prepare('SELECT * FROM report_templates WHERE id = ?').get(id));
    }
    static async list(input) {
        const { page = 1, limit = 20 } = input;
        const offset = (page - 1) * limit;
        const count = (await db.prepare('SELECT count(*) as c FROM report_templates').get()).c;
        const templates = await db.prepare('SELECT * FROM report_templates ORDER BY created_at DESC LIMIT ? OFFSET ?').all(limit, offset);
        return { templates: templates.map(mapTemplate), total: count, page, limit };
    }
    static async getById(id) {
        const row = await db.prepare('SELECT * FROM report_templates WHERE id = ?').get(id);
        if (!row)
            throw new AppError(404, 'Template not found');
        return mapTemplate(row);
    }
    static async update(id, userId, input, role) {
        const existing = await db.prepare('SELECT * FROM report_templates WHERE id = ?').get(id);
        if (!existing)
            throw new AppError(404, 'Template not found');
        if (role !== 'director' && role !== 'hr' && existing.user_id !== userId)
            throw new AppError(403, 'Cannot update others template');
