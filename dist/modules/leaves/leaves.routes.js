...
router.get('/balance', apiCache({ ttl: 60_000 }), async (req, res, next) => {
    try {
        const targetId = req.query.userId || req.user.sub;
...
router.get('/', validate(listLeavesSchema, 'query'), async (req, res, next) => {
    try {
        const result = await LeavesService.list(req.query, req.user.sub, req.user.role);
        res.json(result);
    }
    catch (err) {
        next(err);
...
router.post('/', validate(createLeaveSchema), async (req, res, next) => {
    try {
        const leave = await LeavesService.create(req.user.sub, req.body, req.user.role);
        try {
            await ActivityLogsService.create(req.user.sub, 'apply', 'leave', leave.id, { type: leave.type, startDate: leave.startDate, endDate: leave.endDate }, req.ip);
        }
        catch (e) {
...
router.post('/:id/review', requireRole('director', 'hr'), requireUuid('id'), validate(reviewLeaveSchema), async (req, res, next) => {
    try {
        const leave = await LeavesService.review(req.params.id, req.user.sub, req.body.status, req.body.reviewComment, req.user.role);
        try {
            await ActivityLogsService.create(req.user.sub, 'review', 'leave', leave.id, { status: leave.status }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
