...
export class PlansService {
    static async create(userId, input) {
        ...
        if (existing)
            throw new AppError(409, 'Plan already exists for this date');
        const id = uuid();
        try {
    ...
    static async list(input, userId, role) {
        ...
        const count = (await db.prepare(`SELECT count(*) as c FROM work_plans wp ${where}`).get(...params)).c;
    ...
    static async getById(id, userId, role) {
        const p = await db.prepare('SELECT * FROM work_plans WHERE id = ?').get(id);
        if (!p)
            throw new AppError(404, 'Plan not found');
        if (role === 'employee' && p.user_id !== userId)
            throw new AppError(403, 'Forbidden');
        return mapPlan(p);
    }
    static async update(id, userId, input, role) {
        const plan = await db.prepare('SELECT * FROM work_plans WHERE id = ?').get(id);
        if (!plan)
            throw new AppError(404, 'Plan not found');
        if (role !== 'director' && role !== 'hr' && plan.user_id !== userId)
            throw new AppError(403, 'Cannot update others plan');
        if (plan.status !== 'draft')
            throw new AppError(409, 'Only draft plans can be updated');
    ...
    static async submit(id, userId) {
        const result = await db.transaction(async () => {
            ...
            if (plan.user_id !== userId)
                throw new AppError(403, 'Cannot submit others plan');
            if (plan.status !== 'draft')
                throw new AppError(409, 'Only draft plans can be submitted');
    ...
    static async review(id, reviewedById, status, comments) {
        const result = await db.transaction(async () => {
            ...
            if (plan.status !== 'submitted')
                throw new AppError(409, 'Plan not in reviewable state');
            if (plan.user_id === reviewedById)
                throw new AppError(403, 'Cannot review your own plan');
    ...
    static async delete(id, userId, role) {
        ...
        if (role === 'employee') {
            if (plan.user_id !== userId)
                throw new AppError(403, 'Cannot delete others plan');
            if (plan.status !== 'draft')
                throw new AppError(409, 'Only draft plans can be deleted');
        }
