...
export class FilesService {
    static async upload(userId, file) {
        ...
        const id = uuid();
        await db.prepare(`INSERT INTO file_uploads (id, user_id, original_name, storage_key, mime_type, size_bytes, url) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, userId, file.originalname, storageKey, file.mimetype, size, url);
        const row = await db.prepare('SELECT * FROM file_uploads WHERE id = ?').get(id);
        return mapFile(row);
    }
    static async download(fileId, userId, role) {
        const row = await db.prepare('SELECT * FROM file_uploads WHERE id = ?').get(fileId);
        if (!row)
            throw new AppError(404, 'File not found');
        const isOwner = row.user_id === userId;
        const isAdmin = isAdminRole(role);
        if (!isOwner && !isAdmin) {
            const assigned = await db.prepare(`SELECT 1 FROM task_assignments ta
         JOIN task_attachments att ON att.task_id = ta.task_id
         WHERE att.file_url = ? AND ta.user_id = ?`).get(row.url, userId);
            if (!assigned)
                throw new AppError(403, 'Access denied');
        }
        const storage = getStorage();
        const buffer = await storage.download(row.storage_key);
        await db.prepare(`INSERT INTO activity_logs (id, actor_id, action, entity_type, entity_id) VALUES (?, ?, 'download', 'file', ?)`).run(uuid(), userId, fileId);
        return { buffer, filename: row.original_name, mimeType: row.mime_type };
    }
    static async delete(fileId, userId, role) {
        const row = await db.prepare('SELECT * FROM file_uploads WHERE id = ?').get(fileId);
        if (!row)
            throw new AppError(404, 'File not found');
        const isOwner = row.user_id === userId;
    ...
    static async list(input, role, userId) {
        ...
        const rows = await db.prepare(`SELECT f.* FROM file_uploads f ${where} ORDER BY f.created_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
        const total = (await db.prepare(`SELECT COUNT(*) AS c FROM file_uploads f ${where}`).get(...params)).c;
    ...
    static async cleanupExpiredFiles() {
        const policies = await db.prepare('SELECT * FROM file_retention_policies').all();
        const now = Date.now();
        let deleted = 0;
        for (const policy of policies) {
            ...
            const expired = await db.prepare(`SELECT f.* FROM file_uploads f ${where}`).all(...params);
            const storage = getStorage();
