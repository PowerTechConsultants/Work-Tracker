...
router.get('/', validate(listFilesSchema, 'query'), async (req, res, next) => {
    try {
        const result = await FilesService.list(req.query, req.user.role, req.user.sub);
        res.json(result);
    }
    catch (err) {
        next(err);
...
router.get('/retention', requireRole('director', 'hr'), async (_req, res, next) => {
    try {
...
router.post('/cleanup', requireRole('director', 'hr'), async (_req, res, next) => {
    try {
