...
export class AuthService {
    static async login(input, userAgent, ip) {
        ...
        const user = await db.prepare('SELECT id, email, password_hash, role, status, employee_id, first_name, last_name, designation, department_id, phone_number, joining_date, profile_picture_url, failed_login_attempts, locked_until, two_factor_enabled, two_factor_secret FROM users WHERE email = ?').get(email);
        if (!user)
    ...
    static async handleTwoFactorLogin(user, input, userAgent, ip) {
        const code = input.twoFactorCode?.trim();
        const pendingToken = input.pendingAuthToken;
        if (code && pendingToken) {
            let payload;
            try {
                payload = verifyPendingAuthToken(pendingToken);
            }
            catch {
    ...
    static async recordFailedAttempt(user, email, ip) {
        ...
        if (user.failed_login_attempts + 1 >= SECURITY_ALERT_THRESHOLD) {
            ...
            if (emailAttempts >= SECURITY_ALERT_THRESHOLD) {
                sendSecurityAlertEmail(user.email ?? email, ip).catch(e => console.error('[AUTH] Security alert email failed:', e));
            }
        }
    ...
    static async setupTwoFactor(userId) {
        const user = await db.prepare('SELECT id, email, two_factor_enabled, two_factor_secret FROM users WHERE id = ?').get(userId);
        if (!user)
    ...
    static async verifyAndEnableTwoFactor(userId, input) {
        const user = await db.prepare('SELECT id, two_factor_enabled FROM users WHERE id = ?').get(userId);
        if (!user)
            throw new AppError(404, 'User not found');
        if (user.two_factor_enabled)
            throw new AppError(409, 'Two-factor authentication is already enabled');
    ...
    static async disableTwoFactor(userId, currentPassword) {
        const user = await db.prepare('SELECT id, password_hash, two_factor_enabled FROM users WHERE id = ?').get(userId);
        if (!user)
            throw new AppError(404, 'User not found');
        if (!user.two_factor_enabled)
            throw new AppError(409, 'Two-factor authentication is not enabled');
    ...
    static async refreshToken(rawToken) {
        ...
        const stored = await db.prepare('SELECT * FROM refresh_tokens WHERE user_id = ? AND id = ?').get(payload.sub, payload.tokenId);
        if (!stored)
        ...
        if (parseUTC(stored.expires_at) < new Date())
            throw new AppError(401, 'Refresh token expired');
        if (!verifyTokenHash(rawToken, stored.token_hash))
            throw new AppError(401, 'Token mismatch');
        return await db.transaction(async () => {
            // Atomically check and revoke inside transaction to prevent TOCTOU race
            const current = await db.prepare('SELECT revoked_at FROM refresh_tokens WHERE id = ? FOR UPDATE').get(stored.id);
            ...
            const user = await db.prepare('SELECT id, email, role, status FROM users WHERE id = ?').get(payload.sub);
            if (!user)
    ...
    static async changePassword(userId, input) {
        ...
        if (!user)
            throw new AppError(404, 'User not found');
        if (!await bcryptBreaker.call(() => bcrypt.compare(input.currentPassword, user.password_hash)))
    ...
    static async getMe(userId) {
        const user = await db.prepare('SELECT id, employee_id, first_name, last_name, email, role, department_id, designation, joining_date, status, phone_number, profile_picture_url, two_factor_enabled FROM users WHERE id = ?').get(userId);
        if (!user)
            throw new AppError(404, 'User not found');
        return {
            id: user.id, employeeId: user.employee_id, firstName: user.first_name, lastName: user.last_name,
    ...
    static async resetPassword(token, newPassword) {
        ...
        const row = await db.prepare("SELECT * FROM password_reset_tokens WHERE token_hash = ? AND used_at IS NULL AND expires_at > datetime('now')").get(tokenHash);
        if (!row)
