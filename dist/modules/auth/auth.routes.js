...
router.post('/login', validate(loginSchema), loginLimiter, async (req, res, next) => {
    try {
        const result = await AuthService.login(req.body, req.headers['user-agent'], req.ip);
        if ('twoFactorRequired' in result) {
            try {
                await ActivityLogsService.create(result.user.id, 'login_2fa_pending', 'auth', result.user.id, { email: req.body.email }, req.ip);
            }
            catch (e) {
        ...
        try {
            await ActivityLogsService.create(result.user.id, 'login', 'auth', result.user.id, { email: req.body.email }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/register', authenticate, requireRole('director'), validate(registerSchema), async (req, res, next) => {
    try {
        const user = await AuthService.register(req.body);
        try {
            await ActivityLogsService.create(req.user.sub, 'register', 'auth', user.id, { email: req.body.email, role: user.role }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/logout', async (req, res, next) => {
    try {
        ...
        if (refreshToken) {
            ...
            catch (e) { /* token already invalid, still clear cookies */ }
        }
        res.clearCookie('refreshToken', { path: '/api/v1/auth', httpOnly: true, sameSite: 'lax', secure: isProduction });
        res.clearCookie('accessToken', { path: '/', httpOnly: true, sameSite: 'lax', secure: isProduction });
...
router.post('/change-password', authenticate, validate(changePasswordSchema), async (req, res, next) => {
    try {
        ...
        try {
            await ActivityLogsService.create(req.user.sub, 'change_password', 'auth', req.user.sub, undefined, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/2fa/verify-enable', authenticate, validate(twoFactorVerifySchema), async (req, res, next) => {
    try {
        const result = await AuthService.verifyAndEnableTwoFactor(req.user.sub, req.body);
        try {
            await ActivityLogsService.create(req.user.sub, 'enable_2fa', 'auth', req.user.sub, undefined, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/2fa/disable', authenticate, validate(twoFactorDisableSchema), async (req, res, next) => {
    try {
        const result = await AuthService.disableTwoFactor(req.user.sub, req.body.currentPassword);
        try {
            await ActivityLogsService.create(req.user.sub, 'disable_2fa', 'auth', req.user.sub, undefined, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/forgot-password', forgotPasswordLimiter, validate(forgotPasswordSchema), async (req, res, next) => {
    try {
        ...
        try {
            await ActivityLogsService.create(null, 'forgot_password', 'auth', undefined, { email: req.body.email }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/reset-password', resetPasswordLimiter, validate(resetPasswordSchema), async (req, res, next) => {
    try {
