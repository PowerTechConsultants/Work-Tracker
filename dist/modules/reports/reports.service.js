...
export class ReportsService {
    static async create(userId, input) {
        ...
        if (existing)
            throw new AppError(409, 'Report already exists for this date');
        const id = uuid();
        try {
    ...
    static async list(input, userId, role) {
        ...
        const count = (await db.prepare(`SELECT count(*) as c FROM work_reports wr ${where}`).get(...params)).c;
    ...
    static async getById(id, userId, role) {
        const r = await db.prepare('SELECT * FROM work_reports WHERE id = ?').get(id);
        if (!r)
            throw new AppError(404, 'Report not found');
        if (role === 'employee' && r.user_id !== userId)
            throw new AppError(403, 'Forbidden');
        return mapReport(r);
    }
    static async update(id, userId, input, role) {
        const report = await db.prepare('SELECT * FROM work_reports WHERE id = ?').get(id);
        if (!report)
            throw new AppError(404, 'Report not found');
        if (role !== 'director' && role !== 'hr' && report.user_id !== userId)
            throw new AppError(403, 'Cannot update others report');
        if (report.status !== 'draft')
            throw new AppError(409, 'Only draft reports can be updated');
        const sets = ["updated_at = datetime('now')"];
        const params = [];
        if (input.workCompletedToday !== undefined) {
            sets.push('work_completed_today = ?');
            params.push(input.workCompletedToday);
        }
        if (input.currentProgress !== undefined) {
            sets.push('current_progress = ?');
            params.push(input.currentProgress);
        }
        if (input.pendingWork !== undefined) {
            sets.push('pending_work = ?');
            params.push(input.pendingWork);
        }
        if (input.blockers !== undefined) {
            sets.push('blockers = ?');
            params.push(input.blockers);
        }
        if (input.tomorrowPlan !== undefined) {
            sets.push('tomorrow_plan = ?');
            params.push(input.tomorrowPlan);
        }
        params.push(id);
    ...
    static async submit(id, userId) {
        const result = await db.transaction(async () => {
            ...
            if (report.user_id !== userId)
                throw new AppError(403, 'Cannot submit others report');
            if (report.status !== 'draft')
                throw new AppError(409, 'Only draft reports can be submitted');
    ...
    static async review(id, reviewedById, status, comments) {
        const result = await db.transaction(async () => {
            const report = await db.prepare('SELECT * FROM work_reports WHERE id = ?').get(id);
            if (!report)
                throw new AppError(404, 'Report not found');
            if (report.status !== 'submitted')
                throw new AppError(409, 'Report not in reviewable state');
            if (report.user_id === reviewedById)
                throw new AppError(403, 'Cannot review your own report');
    ...
    static async delete(id, userId, role) {
        ...
        if (role === 'director' || role === 'hr') {
            ...
            if (report.user_id !== userId)
                throw new AppError(403, 'Cannot delete others report');
            if (report.status !== 'draft')
                throw new AppError(409, 'Only draft reports can be deleted');
        }
    ...
    static async exportByUser(userId, startDate, endDate) {
        const conds = [];
        const params = [];
        if (userId) {
            conds.push('wr.user_id = ?');
