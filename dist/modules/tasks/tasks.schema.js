...
export const createTaskSchema = z.object({
    ...
    assigneeIds: z.array(z.string().trim().min(1, 'Invalid assignee ID')).min(1, 'At least one assignee is required').refine((ids) => new Set(ids).size === ids.length, { message: 'Duplicate assignees are not allowed' }),
...
export const updateTaskSchema = z.object({
    ...
    assigneeIds: z.array(z.string().trim().min(1, 'Invalid assignee ID')).optional().refine((ids) => !ids || new Set(ids).size === ids.length, { message: 'Duplicate assignees are not allowed' }),
    estimatedHours: z.number().min(0, 'Hours must be positive').max(1000, 'Hours too high').optional(),
...
export const listTasksSchema = z.object({
    ...
    status: z.enum(['pending', 'in_progress', 'completed', 'on_hold', 'cancelled']).optional(),
    statuses: z.preprocess((val) => (val === undefined || val === null ? val : Array.isArray(val) ? val : [val]), z.array(z.enum(['pending', 'in_progress', 'completed', 'on_hold', 'cancelled'])).optional()),
    priority: z.enum(['low', 'medium', 'high', 'urgent']).optional(),
    priorities: z.preprocess((val) => (val === undefined || val === null ? val : Array.isArray(val) ? val : [val]), z.array(z.enum(['low', 'medium', 'high', 'urgent'])).optional()),
    assigneeId: z.string().trim().min(1).optional(),
    assigneeIds: z.preprocess((val) => (val === undefined || val === null ? val : Array.isArray(val) ? val : [val]), z.array(z.string().trim().min(1)).optional()),
