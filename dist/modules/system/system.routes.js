...
router.get('/db', requireRole('director', 'hr'), async (_req, res, next) => {
    try {
        const tables = (await db.prepare("SELECT TABLE_NAME as name FROM information_schema.tables WHERE TABLE_SCHEMA = DATABASE() ORDER BY TABLE_NAME").all()).map((r) => r.name);
        const tableCounts = [];
        for (const t of tables) {
            if (ALLOWED_TABLES.has(t)) {
                const row = await db.prepare(`SELECT COUNT(*) c FROM \`${t}\``).get();
                tableCounts.push({ table: t, rows: row.c });
            }
        }
        const version = (await db.prepare('SELECT version FROM schema_migrations ORDER BY version DESC LIMIT 1').get())?.version ?? 0;
        res.json({
            journalMode: 'MySQL',
...
router.post('/backup', requireRole('director'), async (req, res, next) => {
    try {
...
router.get('/backups', requireRole('director'), (_req, res, next) => {
    try {
