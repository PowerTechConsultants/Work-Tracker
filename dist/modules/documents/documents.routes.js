...
router.post('/', validate(createDocumentRequestSchema), async (req, res, next) => {
    try {
        const doc = await DocumentsService.create(req.user.sub, req.user.role, req.body);
        try {
            await ActivityLogsService.create(req.user.sub, 'request', 'document', doc.id, { docType: doc.docType, userId: doc.userId }, req.ip);
        }
        catch (e) {
...
router.get('/', validate(listDocumentsSchema, 'query'), async (req, res, next) => {
    try {
        const result = await DocumentsService.list(req.query, req.user.sub, req.user.role);
        res.json(result);
    }
    catch (err) {
        next(err);
...
router.post('/:id/issue', requireRole('director', 'hr'), requireUuid('id'), validate(issueDocumentBodySchema), async (req, res, next) => {
    try {
        const doc = await DocumentsService.issue(req.params.id, req.user.sub, req.user.role, req.body);
        try {
            await ActivityLogsService.create(req.user.sub, 'issue', 'document', doc.id, { docType: doc.docType, docNumber: doc.docNumber }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/:id/reject', requireRole('director', 'hr'), requireUuid('id'), validate(rejectDocumentSchema), async (req, res, next) => {
    try {
        const doc = await DocumentsService.reject(req.params.id, req.user.sub, req.user.role, req.body.reason);
        try {
            await ActivityLogsService.create(req.user.sub, 'reject', 'document', doc.id, { docType: doc.docType, reason: req.body.reason ?? null }, req.ip);
        }
        catch (e) {
            console.error('[ActivityLogs] Failed:', e);
        }
...
router.post('/:id/cancel', requireUuid('id'), async (req, res, next) => {
    try {
...
router.get('/:id/download', requireUuid('id'), async (req, res, next) => {
    try {
