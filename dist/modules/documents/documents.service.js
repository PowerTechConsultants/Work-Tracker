...
import { documentFieldSchemas, DOC_TYPE_LABELS, DOC_TYPE_CODES } from './documents.schema.js';
...
function mapDoc(r) {
    ...
    const label = DOC_TYPE_LABELS[r.doc_type] ?? r.doc_type;
    return {
        id: r.id,
        userId: r.user_id,
...
async function insertNotification(recipientId, senderId, title, message, type, link) {
...
export class DocumentsService {
    static async create(requestedById, role, input) {
        ...
        const label = DOC_TYPE_LABELS[input.docType];
        if (!label)
            throw new AppError(400, 'Unknown document type');
        const target = await db.prepare('SELECT id, first_name, last_name, status FROM users WHERE id = ?').get(targetId);
        if (!target)
            throw new AppError(404, 'Employee not found');
        if (target.status !== 'active')
            throw new AppError(400, 'Employee account is not active');
        const id = uuid();
        await db.transaction(async () => {
            ...
            if (duplicate)
                throw new AppError(409, `A pending ${label} request already exists for this employee`);
        ...
        const requester = await db.prepare('SELECT first_name, last_name FROM users WHERE id = ?').get(requestedById);
        ...
        const admins = await db.prepare("SELECT id FROM users WHERE role IN ('director', 'hr') AND status = 'active'").all();
        for (const a of admins) {
            if (a.id === requestedById)
                continue;
            const payload = {
    ...
    static async list(query, userId, role) {
        ...
        const total = (await db.prepare(`SELECT COUNT(*) AS c FROM document_requests d ${where}`).get(...params)).c;
        const pendingCount = (await db.prepare("SELECT COUNT(*) AS c FROM document_requests WHERE status = 'pending'").get()).c;
        return { documents: rows.map(mapDoc), total, page, limit, pendingCount };
    }
    static async getById(id, userId, role) {
        const row = await db.prepare(`${SELECT} WHERE d.id = ?`).get(id);
        if (!row)
            throw new AppError(404, 'Document request not found');
        if (!isAdminRole(role) && row.user_id !== userId)
            throw new AppError(403, 'Forbidden');
        return mapDoc(row);
    }
    static async issue(id, reviewedById, role, input) {
        if (!isAdminRole(role))
            throw new AppError(403, 'Only directors and HR can issue documents');
        const doc = await db.prepare('SELECT * FROM document_requests WHERE id = ?').get(id);
        if (!doc)
            throw new AppError(404, 'Document request not found');
        if (doc.status !== 'pending')
            throw new AppError(409, `Only pending requests can be issued (current status: ${doc.status})`);
        const fieldSchema = documentFieldSchemas[doc.doc_type];
        if (!fieldSchema)
            throw new AppError(400, 'Unknown document type');
        // Fill any employee fields the reviewer left blank from the live employee record
        const emp = await db.prepare(`SELECT u.first_name, u.last_name, u.employee_id, u.designation, u.joining_date,
              u.dob, u.qualification, u.gender, u.address_street, u.address_city, u.address_state, u.address_pincode,
              u.father_name, u.phone_number, u.email,
              dept.name AS department_name
       FROM users u LEFT JOIN departments dept ON u.department_id = dept.id WHERE u.id = ?`).get(doc.user_id);
        ...
        if (emp) {
            ...
            if (doc.doc_type === 'leaving_certificate' && emp.joining_date && inputFields.relievingDate) {
                const joining = new Date(emp.joining_date);
                const relieving = new Date(inputFields.relievingDate);
            ...
            for (const [key, value] of Object.entries(snapshot)) {
                if (!value)
                    continue; // never inject empty strings - optional fields must stay undefined
        ...
        if (doc.doc_type === 'salary_slip') {
            const f = inputFields;
            if (f.daysWorked !== undefined && f.paidDays === undefined)
                f.paidDays = f.daysWorked;
            if (f.basicPay !== undefined && f.basic === undefined)
                f.basic = f.basicPay;
            if (f.travellingAllowance !== undefined && f.conveyance === undefined)
                f.conveyance = f.travellingAllowance;
            if (f.dearnessAllowance !== undefined && f.hra === undefined)
                f.hra = f.dearnessAllowance;
            if (f.variablePay !== undefined && f.specialAllowance === undefined)
                f.specialAllowance = f.variablePay;
            // lopDays optional, ensure number
            if (f.advancePaid !== undefined && f.otherDeductions === undefined)
                f.otherDeductions = f.advancePaid;
            // Ensure required fields have defaults if still missing
            if (f.paidDays === undefined || f.paidDays === '')
                f.paidDays = 30;
            if (f.basic === undefined || f.basic === '')
                f.basic = 0;
        }
        ...
        const updated = await db.transaction(async () => {
            ...
            const code = DOC_TYPE_CODES[doc.doc_type] ?? 'DOC';
            const seq = (await db.prepare("SELECT COUNT(*) AS c FROM document_requests WHERE doc_type = ? AND status = 'issued' AND YEAR(issued_at) = ?").get(doc.doc_type, year)).c;
            let dn;
        ...
        const recipient = await db.prepare('SELECT id, email, first_name, last_name FROM users WHERE id = ?').get(doc.user_id);
        if (recipient) {
            try {
                await sendDocumentReady({ id: doc.id, docType: doc.doc_type, docNumber }, { id: recipient.id, email: recipient.email, firstName: recipient.first_name, lastName: recipient.last_name });
            }
            catch (e) {
    ...
    static async reject(id, reviewedById, role, reason) {
        if (!isAdminRole(role))
            throw new AppError(403, 'Only directors and HR can reject document requests');
        const doc = await db.prepare('SELECT * FROM document_requests WHERE id = ?').get(id);
        if (!doc)
            throw new AppError(404, 'Document request not found');
        if (doc.status !== 'pending')
            throw new AppError(409, `Only pending requests can be rejected (current status: ${doc.status})`);
    ...
    static async cancel(id, userId, role) {
        const doc = await db.prepare('SELECT id, user_id, status FROM document_requests WHERE id = ?').get(id);
        if (!doc)
            throw new AppError(404, 'Document request not found');
        if (doc.user_id !== userId && !isAdminRole(role))
            throw new AppError(403, 'Forbidden');
        if (doc.status !== 'pending')
    ...
    static async download(id, userId, role) {
        const doc = await this.getById(id, userId, role);
        if (doc.status !== 'issued')
            throw new AppError(409, 'Document has not been issued yet');
