...
export class AnalyticsService {
    static async attendanceTrends(year, month) {
        const start = `${year}-${String(month).padStart(2, '0')}-01`;
        const lastDay = new Date(year, month, 0).getDate();
        const end = `${year}-${String(month).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
        const rows = await db.prepare(`SELECT date, status, count(*) as count FROM attendance
      WHERE date >= ? AND date <= ? GROUP BY date, status ORDER BY date`).all(start, end);
        const daily = {};
        for (const r of rows) {
            if (!daily[r.date])
                daily[r.date] = {};
    ...
    static async departmentStats() {
      ...
      LEFT JOIN users u ON u.department_id = d.id AND u.status = 'active' GROUP BY d.id ORDER BY d.name`).all();
        const today = getISTDate();
        const attendanceRows = await db.prepare(`SELECT u.department_id, a.status, count(*) as count FROM attendance a
      JOIN users u ON a.user_id = u.id WHERE a.date = ? AND u.department_id IS NOT NULL GROUP BY u.department_id, a.status`).all(today);
        ...
        for (const r of attendanceRows) {
            ...
            if (stat) {
                if (r.status === 'present' || r.status === 'work_end' || r.status === 'on_break' || r.status === 'half_day' || r.status === 'remote')
                    stat.present += r.count;
                else if (r.status === 'absent')
                    stat.absent += r.count;
                else if (r.status === 'leave')
                    stat.leave += r.count;
                else if (r.status === 'holiday')
                    stat.holiday += r.count;
            }
        }
        return {
            totalUsers,
            departments: Array.from(deptMap.entries()).map(([id, s]) => {
                ...
                return {
                    id, name: deptRows.find((d) => d.id === id)?.name ?? 'Unknown',
    ...
    `).all(start, end)).map((h) => h.date);
        for (const l of leaves) {
            const parseDate = (s) => new Date(`${s}T00:00:00.000Z`);
    ...
    static async taskSummary() {
        const total = (await db.prepare('SELECT count(*) as c FROM tasks').get()).c;
        const byStatus = await db.prepare('SELECT status, count(*) as count FROM tasks GROUP BY status').all();
        const today = getISTDate();
        const overdue = (await db.prepare("SELECT count(*) as c FROM tasks WHERE status NOT IN ('completed','cancelled') AND date(due_date) < ?").get(today)).c;
        const completionRate = total > 0 ? Math.round(((byStatus.find((s) => s.status === 'completed')?.count ?? 0) / total) * 100) : 0;
        return { total, byStatus, overdue, completionRate };
    }
    static async overtimeAnalytics(year) {
        const start = `${year}-01-01`;
        const end = `${year}-12-31`;
    ...
    `).all(start, end);
        ...
        for (const r of rows) {
            ...
            const entry = monthlyMap.get(monthKey);
            if (entry)
                entry[r.dept_name ?? 'Unassigned'] = (entry[r.dept_name ?? 'Unassigned'] ?? 0) + r.total_overtime;
        }
        const departments = [...new Set(rows.map((r) => r.dept_name ?? 'Unassigned'))];
        ...
        const totals = departments.reduce((acc, dept) => {
            acc[dept] = rows.filter((r) => (r.dept_name ?? 'Unassigned') === dept).reduce((s, r) => s + r.total_overtime, 0);
            return acc;
        }, {});
        return { monthly, departments, totals };
    }
    static async productivity(userId, startDate, endDate) {
        const today = getISTDate();
    ...
    `).all(monthStart, monthEnd);
      ...
      GROUP BY u.department_id
    `).all(monthStart, monthEnd);
        const attMap = new Map(attendanceRows.map((r) => [r.department_id, r]));
        const taskMap = new Map(taskRows.map((r) => [r.department_id, r]));
        const leaveMap = new Map(leaveRows.map((r) => [r.department_id, r]));
        const otMap = new Map(overtimeRows.map((r) => [r.department_id, r]));
        return {
    ...
    static async employeeProductivity(departmentId, period = 'month') {
        const today = getISTDate();
        const now = getISTNow();
        let start;
        switch (period) {
    ...
    `).all(...userIds, start, today);
      ...
      GROUP BY user_id
    `).all(...userIds, start, today);
        const taskMap = new Map(taskStats.map((r) => [r.user_id, r]));
        const attMap = new Map(attStats.map((r) => [r.user_id, r]));
        ...
        employees.sort((a, b) => b.efficiency - a.efficiency);
        return { period, employees };
    }
    static async managerDashboard(managerId) {
        const today = getISTDate();
        const now = getISTNow();
        const dow = now.getDay();
        const offset = dow === 0 ? 6 : dow - 1;
        ...
        const teamRows = await db.prepare('SELECT DISTINCT team_name FROM team_members WHERE user_id = ?').all(managerId);
        const teamNames = teamRows.map((r) => r.team_name);
    ...
    `).all(today, ...memberIds);
        ...
        for (const a of todayAtt) {
            ...
            if (['present', 'work_end', 'on_break', 'half_day', 'remote'].includes(a.status))
                present++;
    ...
    `).get(...memberIds))?.c ?? 0;
      ...
      GROUP BY ta.user_id
    `).all(weekStart, ...memberIds);
        const workloadMap = new Map(workload.map((w) => [w.user_id, w]));
