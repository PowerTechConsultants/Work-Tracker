import { z } from 'zod';
const attendanceStatuses = ['present', 'on_break', 'work_end', 'absent', 'leave', 'half_day', 'holiday', 'remote'];
const checkInStatuses = ['present', 'remote'];
...
export const listAttendanceSchema = z.object({
    ...
    userIds: z.preprocess((val) => (val === undefined || val === null ? val : Array.isArray(val) ? val : [val]), z.array(z.string().trim().min(1)).optional()),
    ...
    statuses: z.preprocess((val) => (val === undefined || val === null ? val : Array.isArray(val) ? val : [val]), z.array(z.enum(attendanceStatuses)).optional()),
});
