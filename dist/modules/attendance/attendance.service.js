...
function emitAttendanceUpdated(userId, record) {
    try {
...
async function logEvent(attendanceId, userId, eventType, loc, occurredAt) {
    try {
        await db.prepare("INSERT INTO attendance_events (id, attendance_id, user_id, event_type, occurred_at, latitude, longitude, location_accuracy, location_captured_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").run(uuid(), attendanceId, userId, eventType, occurredAt ?? new Date().toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, ''), loc?.latitude ?? null, loc?.longitude ?? null, loc?.accuracy ?? null, loc?.locationCapturedAt ?? null);
    }
    catch (e) {
        console.error('[Attendance] Event log failed:', e);
    }
}
export async function getAttendanceEvents(attendanceId) {
    return await db.prepare("SELECT id, event_type, occurred_at, latitude, longitude, location_accuracy, location_captured_at FROM attendance_events WHERE attendance_id = ? ORDER BY occurred_at ASC").all(attendanceId);
}
export async function getAttendanceEventsForUser(userId, attendanceId) {
    return await db.prepare("SELECT id, event_type, occurred_at, latitude, longitude, location_accuracy, location_captured_at FROM attendance_events WHERE attendance_id = ? AND user_id = ? ORDER BY occurred_at ASC").all(attendanceId, userId);
}
export async function getPauseLog(startDate, endDate) {
    const conds = ["ae.event_type IN ('pause_start', 'pause_end')"];
    const params = [];
    if (startDate) {
        conds.push('a.date >= ?');
...
export class AttendanceService {
    static async checkIn(userId, input) {
        ...
        if (existing) {
            if (existing.logout_time)
                throw new AppError(409, 'Already checked out today');
            if (existing.status === 'holiday')
                throw new AppError(403, 'Cannot check in on a holiday');
            if (existing.status === 'leave')
                throw new AppError(403, 'Cannot check in — approved leave for today');
            ...
            if (existing.login_time)
                throw new AppError(409, 'Already checked in today');
        }
        try {
            if (hasLocation) {
            ...
            if (err.message?.includes('UNIQUE constraint') || err.message?.includes('Duplicate entry') || err.message?.includes('ER_DUP_ENTRY'))
                throw new AppError(409, 'Already checked in today');
            throw err;
        }
    ...
    static async startPause(userId, input) {
        const today = getISTDate();
        const rec = await db.prepare('SELECT * FROM attendance WHERE user_id = ? AND date = ?').get(userId, today);
        if (!rec)
            throw new AppError(404, 'No check-in found for today');
        if (!rec.login_time)
            throw new AppError(400, 'No check-in found for today');
        if (rec.logout_time)
            throw new AppError(409, 'Already checked out today');
    ...
    static async endPause(userId, input) {
        let today = getISTDate();
        let rec = await db.prepare('SELECT * FROM attendance WHERE user_id = ? AND date = ?').get(userId, today);
        if (!rec) {
            const istNow = getISTNow();
            istNow.setUTCDate(istNow.getUTCDate() - 1);
            const yesterdayStr = istNow.toISOString().split('T')[0];
            rec = await db.prepare('SELECT * FROM attendance WHERE user_id = ? AND date = ?').get(userId, yesterdayStr);
            if (rec)
                today = yesterdayStr;
        }
        if (!rec)
            throw new AppError(404, 'No check-in found');
        if (!rec.pause_start_time || rec.pause_end_time)
            return rec;
        const now = new Date();
    ...
    static async checkOut(userId, input) {
        let today = getISTDate();
        let rec = await db.prepare('SELECT * FROM attendance WHERE user_id = ? AND date = ?').get(userId, today);
        if (!rec) {
            const istNow = getISTNow();
        ...
        if (!rec)
            throw new AppError(404, 'No check-in found');
        if (!rec.login_time)
            throw new AppError(400, 'No check-in found');
        if (rec.logout_time)
            throw new AppError(409, 'Already checked out today');
        const now = new Date();
        const nowIso = now.toISOString().replace('T', ' ').replace(/\.\d{3}Z$/, '');
    ...
    static async list(input) {
        ...
        const where = conds.length > 0 ? `WHERE ${conds.join(' AND ')}` : '';
        const count = (await db.prepare(`SELECT count(*) as c FROM attendance a ${where}`).get(...params)).c;
    ...
    static async getUserHistory(userId, year, month, startDate, endDate) {
    ...
    `).all(userId, start, end);
      ...
      FROM attendance WHERE user_id = ? AND date >= ? AND date <= ? AND logout_time IS NOT NULL`).get(userId, start, end);
        return {
            year, month,
            records,
            summary: records.reduce((acc, r) => { acc[r.status] = (acc[r.status] || 0) + 1; return acc; }, {}),
    ...
    static async bulkDelete(input) {
        ...
        const where = conds.length > 0 ? `WHERE ${conds.join(' AND ')}` : '';
        const count = (await db.prepare(`SELECT COUNT(*) as c FROM attendance ${where}`).get(...params)).c;
        let deleted = 0;
        if (count > 0) {
    ...
    static async update(id, input) {
        if (!await db.prepare('SELECT id FROM attendance WHERE id = ?').get(id))
        ...
        if (input.loginTime) {
            ...
            params.push(input.loginTime);
        }
        if (input.logoutTime) {
            sets.push('logout_time = ?');
            params.push(input.logoutTime);
        }
        if (input.workingHours !== undefined) {
            if (input.workingHours < 0 || input.workingHours > 24)
                throw new AppError(400, 'Working hours must be between 0 and 24');
            sets.push('working_hours = ?');
            params.push(input.workingHours);
        }
        if (input.overtimeHours !== undefined) {
            if (input.overtimeHours < 0 || input.overtimeHours > 24)
                throw new AppError(400, 'Overtime hours must be between 0 and 24');
            sets.push('overtime_hours = ?');
            params.push(input.overtimeHours);
        }
        if (input.pauseStartTime !== undefined) {
            sets.push('pause_start_time = ?');
            params.push(input.pauseStartTime);
        }
        if (input.pauseEndTime !== undefined) {
            sets.push('pause_end_time = ?');
            params.push(input.pauseEndTime);
        }
        if (input.pauseMinutes !== undefined) {
            if (input.pauseMinutes < 0 || input.pauseMinutes > 480)
                throw new AppError(400, 'Pause minutes must be between 0 and 480');
            sets.push('pause_minutes = ?');
            params.push(input.pauseMinutes);
        }
        ...
        if (input.accuracy !== undefined) {
            sets.push('location_accuracy = ?');
            params.push(input.accuracy);
        }
        if (input.locationCapturedAt !== undefined) {
            sets.push('location_captured_at = ?');
            params.push(input.locationCapturedAt);
        }
        params.push(id);
    ...
    static async getMonthlySummary(userId, year, month) {
        ...
        const statusRows = await db.prepare('SELECT status, count(*) as count FROM attendance WHERE user_id = ? AND date >= ? AND date <= ? GROUP BY status').all(userId, start, end);
        const summary = {};
      ...
      FROM attendance WHERE user_id = ? AND date >= ? AND date <= ? AND logout_time IS NOT NULL`).get(userId, start, end);
        ...
        const overtimeRow = await db.prepare('SELECT standard_hours, actual_hours, overtime_hours FROM monthly_overtime WHERE user_id = ? AND year = ? AND month = ?').get(userId, year, month);
    ...
    static async calculateMonthlyOvertime(userId, year, month) {
        const start = `${year}-${String(month).padStart(2, '0')}-01`;
        const lastDay = new Date(year, month, 0).getDate();
        const end = `${year}-${String(month).padStart(2, '0')}-${String(lastDay).padStart(2, '0')}`;
        const agg = await db.prepare(`SELECT
      ...
      FROM attendance WHERE user_id = ? AND date >= ? AND date <= ? AND logout_time IS NOT NULL`).get(userId, start, end);
        ...
        const allHolidays = await db.prepare('SELECT id, date FROM holidays WHERE date >= ? AND date <= ?').all(start, end);
    ...
    `).all(start, end);
        const assigneesByHoliday = new Map();
        ...
        const id = uuid();
        await db.prepare("INSERT INTO monthly_overtime (id, user_id, year, month, standard_hours, actual_hours, overtime_hours, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW(), NOW()) ON DUPLICATE KEY UPDATE standard_hours = VALUES(standard_hours), actual_hours = VALUES(actual_hours), overtime_hours = VALUES(overtime_hours), updated_at = NOW()").run(id, userId, year, month, standardHours, actualHours, overtimeHours);
    }
    static async getMonthlyOvertime(userId, year, month) {
    ...
    static async getMonthlyOvertimeAll(year, month) {
        return await db.prepare(`
    ...
    static async recalculateAllOvertime(year, month) {
