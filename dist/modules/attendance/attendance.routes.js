...
router.post('/check-out', validate(locationOptionalSchema), async (req, res, next) => {
    try {
...
router.post('/pause-start', validate(locationOptionalSchema), async (req, res, next) => {
    try {
...
router.post('/pause-end', validate(locationOptionalSchema), async (req, res, next) => {
    try {
...
router.get('/events/:attendanceId', requireUuid('attendanceId'), async (req, res, next) => {
    try {
...
router.get('/pause-log', requireRole('director', 'hr'), async (req, res, next) => {
    try {
...
router.get('/today', apiCache({ ttl: 30_000 }), async (req, res, next) => {
    try {
...
router.get('/today-all', requireRole('director', 'hr'), apiCache({ ttl: 30_000 }), async (req, res, next) => {
    try {
...
router.get('/history/:userId', requireRole('director', 'hr'), requireUuid('userId'), async (req, res, next) => {
    try {
        const year = Math.min(2100, Math.max(2000, parseInt(req.query.year) || new Date().getFullYear()));
...
router.get('/monthly', apiCache({ ttl: 60_000 }), validate(monthlyQuerySchema, 'query'), async (req, res, next) => {
    try {
        const q = req.query;
...
router.get('/my', validate(listAttendanceSchema, 'query'), async (req, res, next) => {
    try {
...
router.get('/', requireRole('director', 'hr'), validate(listAttendanceSchema, 'query'), async (req, res, next) => {
    try {
...
router.patch('/:id', requireRole('director', 'hr'), requireUuid('id'), validate(updateAttendanceSchema), async (req, res, next) => {
    try {
...
router.delete('/bulk', requireRole('director'), validate(deleteAttendanceSchema), async (req, res, next) => {
    try {
...
router.get('/overtime/:year/:month', requireRole('director', 'hr'), async (req, res, next) => {
    try {
...
router.get('/overtime/my/:year/:month', async (req, res, next) => {
    try {
        const year = Math.min(2100, Math.max(2000, parseInt(req.params.year ?? '') || new Date().getFullYear()));
        const month = Math.min(12, Math.max(1, parseInt(req.params.month ?? '') || new Date().getMonth() + 1));
