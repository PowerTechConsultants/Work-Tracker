...
try {
    ...
    console.warn('[SETUP] Admin bootstrap skipped (DB not ready, will retry on next health check):', e?.message ?? e);
}
...
const overtimeTimer = setInterval(async () => {
    ...
    if (isLastDay && isNearMidnight) {
        const lastCalc = await db.prepare("SELECT value FROM app_settings WHERE `key` = 'monthly_overtime_last_calc'").get();
        ...
        if (lastCalc?.value !== calcKey) {
            ...
            try {
                ...
                console.error('[Overtime] Calculation error:', e);
            }
        }
    }
}, 60 * 1000);
