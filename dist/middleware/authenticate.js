...
const userCacheCleanupInterval = setInterval(() => {
    const now = Date.now();
...
export function stopUserCacheCleanup() {
...
export async function authenticate(req, res, next) {
    ...
    try {
        ...
        const user = await db.prepare('SELECT status, role FROM users WHERE id = ?').get(req.user.sub);
