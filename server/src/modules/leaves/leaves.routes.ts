...
router.post('/', validate(createLeaveSchema), async (req: Request, res: Response, next) => {
  try {
    ...
    try { ActivityLogsService.create(req.user!.sub, 'apply', 'leave', leave.id, { type: leave.type, startDate: leave.startDate, endDate: leave.endDate }, req.ip); } catch {}
...
router.post('/:id/review', requireRole('director', 'hr'), validate(reviewLeaveSchema), async (req: Request, res: Response, next) => {
  try {
    ...
    try { ActivityLogsService.create(req.user!.sub, 'review', 'leave', leave.id, { status: leave.status }, req.ip); } catch {}
    res.json(leave);
  } catch (err) { next(err); }
});
